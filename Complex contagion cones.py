...
for level in range(0,nLevels): #Building it level by level, from the highest level to the lowest
    ringlevel = nx.watts_strogatz_graph(nNodes[level],nRingNeighbors[level],0) #Making the Watts-Strogatz ring for this level
    for cone in range(0,nCones): #Building this ring for every cone
        mapping = {}
        for node in range(0,nNodes[level]):
